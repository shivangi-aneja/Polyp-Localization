...
import os
import argparse
from localization.ssd_keras.models.keras_ssd300 import ssd_300
from localization.ssd_keras.misc_utils import config_ssd300 as Config
from localization.ssd_keras.keras_loss_function.keras_ssd_loss import SSDLoss
...
from localization.ssd_keras.ssd_encoder_decoder.ssd_input_encoder import SSDInputEncoder
...
from localization.ssd_keras.data_generator.data_augmentation_chain_original_ssd import SSDDataAugmentation
...
parser.add_argument('-tf', '--tf_logs', type=str, default='ssd300',
...
def main():
    ...
    if create_new_model:
        ...
        model = ssd_300(image_size=(Config.img_height, Config.img_width, Config.img_channels),
    ...
    model_checkpoint = ModelCheckpoint(
        filepath=os.getcwd() + '/weights/ssd300_epoch-{epoch:02d}_loss-{loss:.4f}_val_loss-{val_loss:.4f}.h5',
        monitor='val_loss',
        verbose=1,
        save_best_only=True,
    ...
    for val in range(val_dataset_size):
        ...
        image = plt.gcf()
        plt.draw()
        image.savefig(os.getcwd() + "/val_ssd300/val_" + str(val) + ".png", dpi=100)
