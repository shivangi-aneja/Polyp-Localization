...
from localization.ssd_keras.bounding_box_utils.bounding_box_utils import iou, convert_coordinates
from localization.ssd_keras.ssd_encoder_decoder.matching_utils import match_bipartite_greedy, match_multi
...
class SSDInputEncoder:
    ...
    def __init__(self,
        ...
        if scales:
            if (len(scales) != predictor_sizes.shape[
                0] + 1):  # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError(
        ...
        if not (aspect_ratios_per_layer is None):
            if (len(aspect_ratios_per_layer) != predictor_sizes.shape[
                0]):  # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError(
        ...
        for i in range(batch_size):  # For each batch item...
            ...
            if np.any(labels[:, [xmax]] - labels[:, [xmin]] <= 0) or np.any(labels[:, [ymax]] - labels[:, [ymin]] <= 0):
                raise DegenerateBoxError(
                    "SSDInputEncoder detected degenerate ground truth bounding boxes for batch item {} with bounding boxes {}, ".format(
                        i, labels) +
